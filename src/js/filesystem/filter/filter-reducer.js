...
export default function reducer(
  state: Map<string, *> = fromJS(INITIAL_STATE),
  ...
  switch (action.type) {
    case App.actiontypes.APP_CHANGE_PATH:
      ...
      if (action.payload.peak && action.payload.pathRoute.length > 1) {
        newFocus = action.payload.pathRoute[
          action.payload.pathRoute.length - 2
        ];
        ...
        newFocus = action.payload.pathRoute[
          action.payload.pathRoute.length - 1
        ];
      }
    ...
    case Selection.actiontypes.SET_SELECTION:
      if (action.payload.root != state.get("focusedPath")) {
        return state
          .set("focusedPath", action.payload.root)
          .deleteIn(["focused", "userInput"]);
      } else {
        return state;
      }
