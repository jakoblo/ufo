...
export default class ProgressArrow extends React.Component {
  ...
  renderArrow = (
    ...
    return (
      ...
      >
        <clipPath id="clip">
          <rect x="0" y="0" width={arrowWidth} height={progressHeight} />
        </clipPath>
        <g clipPath="url(#clip)">
          ...
          {finished
            ? this.checkIcon(
                0,
                itemHeight * this.props.sourceCount - 6,
                color,
                bgColor
              )
            : null}
  ...
  arrowPartStart = (x: number, y: number, color: string) => {
      ...
      <path
        d={
          "M" +
            ...
            "," +
            (y + 16.858) +
            "l0.002,-0.112c0.024,-0.737 0.174,-1.463 0.462,-2.144c0.296,-0.699 0.727,-1.338 1.263,-1.874c0.605,-0.605 1.34,-1.076 2.143,-1.37c0.65,-0.238 1.334,-0.354 2.025,-0.358l2.735,0c0,0.667 0,1.333 0,2c-0.961,0 -1.922,-0.028 -2.883,0.003c-0.083,0.004 -0.166,0.009 -0.248,0.017c-0.448,0.049 -0.884,0.168 -1.29,0.364c-0.908,0.437 -1.626,1.228 -1.973,2.174c-0.151,0.414 -0.227,0.849 -0.236,1.29c-0.01,1.665 -0.021,4.248 -0.028,7.152l-1.972,0l0,-7.142Z"
        }
        style={styles}
      />
    );
  ...
  arrowPartBetween = (x: number, y: number, color: string) => {
    let styles = { fill: color };
    return (
      <path
        key={y}
  ...
  arrowPartEnd = (x: number, y: number, color: string) => {
    let styles = { fill: color };
    return (
      <path
        d={
  ...
  checkIcon = (x: number, y: number, color: string, bgColor: string) => {
  ...
  errorIcon = (x: number, y: number, color: string, bgColor: string) => {
    let stylesCheck = { fill: color };
    let stylesBg = { fill: bgColor };
    return (
      <g>
